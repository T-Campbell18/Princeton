...
struct
  let d = ref (D.add 1 1 (D.singleton 0 0))
  let rec fib (n:int) : int =
    match D.mem n !d with
    | true -> D.find n !d
    | false -> let result = fib (n-1) + fib (n-2) in
      d := D.add n result !d;
      result
end
...
struct
  ...
  let fib2 (f:int -> int) (n:int): int =
    if (n > 1) then f (n-1) + f (n-2)
    else n
  let fib3 = AutoMemoizer.memo fib2
  let fib (n:int) : int = fib3 n
...
let experiment (n:int) : unit =
  ...
  let manual = time_fun ManualMemoedFib.fib (* CHANGE THIS! *) in
  let automated = time_fun AutoMemoedFib.fib (* CHANGE THIS! *) in
