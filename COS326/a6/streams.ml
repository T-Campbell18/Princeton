...
let rec zeros : num spread_sheet = (* change me! *)
  let rec aux = lazy(Cons(zero, aux)) in
  lazy(Cons(aux,zeros))
...
let map_all (f:num -> num -> 'a -> 'b) (ss:'a spread_sheet) : 'b spread_sheet =
  let rec map_row i j s = lazy(Cons(f i j (head s), map_row i ((+/) j one) (tail s))) in
  let rec map_sheet i ss = lazy(Cons(map_row i zero (head ss), map_sheet ((+/) i one) (tail ss))) in
  map_sheet zero ss


(* create an infinite multiplication table in which every cell contains the
 * product of its indices *)
let multiplication_table = (* change me *)
  map_all (fun i j xij -> ( */ ) i j) zeros
