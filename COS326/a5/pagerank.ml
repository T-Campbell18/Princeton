...
struct
  ...
  let getRandom (ls: 'a list) : 'a =
    match ls with
    | [] -> raise Err
    | _ -> let n = (Random.int (List.length ls)) in List.nth ls n

  let rec randomWalk (i : int) (n : G.node) (ns: NS.node_score_map) (g : G.graph) : NS.node_score_map =
    if i = 0 then ns else
      let ns2 = NS.add_score ns n 1.0 in
      match G.neighbors g n with
      | None -> raise Err
      | Some [] -> (
          match (G.get_random_node g) with
          | None -> raise Err
          | Some n2 -> randomWalk (i - 1) n2 ns2 g)
      | Some ls ->
        match P.do_random_jumps with
        | None -> randomWalk (i - 1) (getRandom ls) ns2 g
        | Some alpha ->
          if alpha >= Random.float 1.0 then
            match (G.get_random_node g) with
            | None -> raise Err
            | Some n2 -> randomWalk (i - 1) n2 ns2 g
          else randomWalk (i - 1) (getRandom ls) ns2 g

  let rank (g : G.graph) : NS.node_score_map =
    match G.get_random_node g with
    | None -> NS.empty
    | Some n -> NS.normalize (randomWalk P.num_steps n NS.empty g)
