...
struct
  ...
  let rec fold (f: key -> value -> 'a -> 'a) (u: 'a) (d: dict) : 'a =
    match d with
    | Leaf -> u
    | Two (l,(k,v),r) -> f k v (fold f (fold f u l) r)
    | Three (l,(k1,v1), m, (k2,v2),r)->
      f k1 v1 (fold f (f k2 v2 (fold f (fold f u l) m)) r)
  ...
  let rec insert_downward (d: dict) (k: key) (v: value) : kicked =
    ...
    | Two(l,n,r) ->
      insert_downward_two (k,v) n l r (* mutual recursion *)
    | Three(l,n1,m,n2,r) ->
      insert_downward_three (k,v) n1 n2 l m r (* mutual recursion *)
   ...
   * ANSWER:
   *    We are testing that our tree is balanced by iterating through
   *    the subtrees and checking if any are unbalanced--i.e. terminate
   *    with a Leaf and another tree (in the case of Two trees) or
   *    some combination of leaves and trees in the case of Three trees.
   *    If two child trees are found at the nodes of a tree, then
   *    they are recursively checked for being balanced.
  *)
  ...
  let rec balanced (d: dict) : bool =
    ...
    | Three (l,_,m,_,r) -> (
      ...
      | _,Leaf,_ -> false
      | Leaf,_,_ -> false
      | _,_,_ -> balanced l && balanced m && balanced r)
  ...
  let test_insert () =
    ...
    let d2 = insert d k v in
    assert(balanced d);
    assert(balanced d2);
    let d = insert_list_reversed empty pairs in
    assert(balanced d);
    let (k,v) = D.gen_pair () in
    let (k1,v1) = (D.gen_key_lt k (), D.gen_value ()) in
    let (k2,v2) = (D.gen_key_gt k (), D.gen_value ()) in
    let (k3,v3) = (D.gen_key_lt k1 (), D.gen_value ()) in
    let d = insert empty k v in
    let d2 = insert d k1 v1 in
    let d3 = insert d2 k2 v2 in
    let d4 = insert d3 k3 v3 in
    assert(balanced d);
    assert(balanced d2);
    assert(balanced d3);
    assert(balanced d4);
  ...
  let test_lookup () =
    ...
    let (k,v) = D.gen_pair () in
    let d2 = insert d k v in
    let k1 = D.gen_key_lt k () in
    assert (lookup (insert empty k v) k1 = None);
    assert(lookup (insert empty k v) k = Some(v));
  ...
  let test_member () =
    ...
    let (k,v) = D.gen_pair () in
    let d2 = insert d k v in
    assert(member empty k = false);
    assert(member d2 k);
    assert(member (insert empty k v) k);
    ()
