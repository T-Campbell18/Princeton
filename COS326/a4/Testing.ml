 ...
 *)
let map = Rec("map", "fun", Rec("_", "list",
        Match (Var "list", EmptyList, "hd", "tl", Cons(App(Var "fun", Var "hd"),
              App(App(Var "map", Var "fun"), Var "tl")))))

(* Replace the constant "one" below with your implementation of
   the function plus1 that adds one to an integer *)
let plus1 = Rec("plus1", "x", Op(Var "x", Plus, one))
...
*)
let incr_all = Rec("incr_all", "ls", App(App(map, plus1), Var "ls"));;
...
let sum_pairs =  Rec("sum_pairs", "list", Match (Var "list", EmptyList,
                  "hd", "tl", Cons(Op(Fst(Var "hd"), Plus, Snd(Var "hd")),
                                            App(Var "sum_pairs", Var "tl"))))
...
let tests = [zero; fact4; list4; sl4; incr_all; clo]
