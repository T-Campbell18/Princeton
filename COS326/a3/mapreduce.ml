...
Name: Tyler Campbell
Netid: tylercc
Minutes Spent on Problem 1.1: 30 min
...
let num_occurs (n:int) (nums:int list) : int =
  foldl (fun acc num -> if num = n then acc + 1 else acc) 0 nums
...
let super_sum (nlists:int list list) : int =
  foldl (fun acc ls -> foldl (+) acc ls) 0 nlists
...
(* @ operator using foldr *)
let append (xs:'a list) (ys:'a list) =
  foldr (fun x y -> x :: y) xs ys
...
let consec_dedupe (eq:'a -> 'a -> bool) (xs:'a list) : 'a list =
  ...
  | _ :: [] -> xs
  | hd::tl->
    let (_,t) = foldl (fun (x,y) curr ->
        if (eq x curr) then (x,y)
        else (curr, append y [curr])) (hd,[hd]) tl
    in t
...
let prefixes (xs: 'a list) : 'a list list =
  ...
  | hd :: [] -> [[hd]]
  | hd :: tl ->
    let (_,t) = foldl (fun (x,y) var -> append x [var],
                            append y [append x [var]]) ([hd],[[hd]]) tl
    in t
