						  ...
						  Chunk_T aoBins[], int iBinCount)
{
	...
	/* Does oHeapStart have non-NULL value? */
	if (oHeapStart == NULL)
	{
		fprintf(stderr, "The heap start is uninitialized\n");
		return FALSE;
	}
	/* Does oHeapEnd have non-NULL value? */
	if (oHeapEnd == NULL)
	{
	...
	if (oHeapStart == oHeapEnd)
	{
		for (iBinCounter = 0; iBinCounter < iBinCount; iBinCounter++)
		{
			...
			{
				fprintf(stderr, "The heap is empty, but "
				"the list is not (bin %d)\n", iBinCounter);
				return FALSE;
			}
		}
		return TRUE;
	...
	for (iBinCounter = 0; iBinCounter < iBinCount; iBinCounter++)
	{
		/* Is the list devoid of forward cycles? */
		oLastChunk = NULL;
		...
		/* Is the list devoid of backward cycles? */
	...
	/* Does each chunk within each bin have the correct number
	of units? */
	for (iBinCounter = 0; iBinCounter < iBinCount - 1; iBinCounter++)
	{
		for (oChunk = aoBins[iBinCounter];
			  ...
			  oChunk = Chunk_getNextInList(oChunk))
		{
			uBinUnits = (int) Chunk_getUnits(oChunk);
			if (uBinUnits != iBinCounter)
			{
				if (iBinCounter != iBinCount - 1 || uBinUnits < iBinCount- 1)
					fprintf(stderr, "bin %d has chunk with incorrect units(%d)\n",
					iBinCounter, uBinUnits);
				return FALSE;
			}
		}	
	}
		  ...
		  oChunk = Chunk_getNextInMem(oChunk, oHeapEnd))
	{
		if (Chunk_getStatus(oChunk) == CHUNK_FREE)
		{
			...
			for (iBinCounter = 0; (iBinCounter < iBinCount)
			&& (oCurrent != oChunk); iBinCounter++)
			{
