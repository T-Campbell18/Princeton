...
/*                                                                     */
/* Then allocate one thousand, 6 field structures then free everything */
/*                                                                     */
...
/* Allocate Dummy Structures and free them */
static void allocFreeDummyStructs(void){
   /* Dummy structure definition */
   enum {NUMBER_OF_DUMMY_STRUCTS = 1000};
   int i;
   struct DummyStruct {
      size_t DummyField1;
      size_t DummyField2;
   ...
   struct DummyStruct *DummyStructArray[NUMBER_OF_DUMMY_STRUCTS];

   /* Allocate NUMBER_OF_DUMMY_STRUCTS structures and then free them */
   for (i = 0; i < NUMBER_OF_DUMMY_STRUCTS; i++)
   {
      DummyStructArray[i] = (struct DummyStruct*)HeapMgr_malloc(sizeof(struct DummyStruct));
      if (DummyStructArray[i] == NULL)
         exit(EXIT_FAILURE);
   }
   for (i = 0; i < NUMBER_OF_DUMMY_STRUCTS; i++)
      HeapMgr_free(DummyStructArray[i]);
}
...
/* Read words from files 06file0.txt to 06file16.txt one file at a time. Write the 25 most
