 ...
 */
static Chunk_T HeapMgr_coalesce(Chunk_T oChunk, int x)
{
	Chunk_T oTmp;
	size_t uUnits;
	...
	/* remove the current chunk from the free list */
	HeapMgr_removeFromList(oChunk);
		
	/* remove the previous chunk in memory from the free list */
	HeapMgr_removeFromList(oTmp);
